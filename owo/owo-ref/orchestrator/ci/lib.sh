...
# url_encode <string>
# Minimal URL encoder (good enough for GitLab project paths with / and spaces)
url_encode() {
  local s="${1:?string required}"
  jq -nr --arg orig "$s" '$orig|@uri'
}

# get_project_id_by_path <namespace/project>
get_project_id_by_path() {
  require_bin jq
  local path="${1:?project path required}"
  local enc
  enc="$(url_encode "$path")"
  api_get "${CI_API_V4_URL}/projects/${enc}" | jq -r '.id'
}
